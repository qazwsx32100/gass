...
-- Add monthly_gas_price column to erp_gas_inventory_periods
alter table public.erp_gas_inventory_periods add column if not exists monthly_gas_price numeric not null default 0;

-- Re-define erp_refresh_relational_mirror to include erp_gas_purchases and monthly_gas_price column update
...
begin
  ...
  for item in select value from jsonb_array_elements(coalesce(p_state->'gasPurchases', '[]'::jsonb)) loop
    insert into public.erp_gas_purchases (id, company_id, purchase_date, qty_50kg, qty_20kg, qty_16kg, qty_10kg, qty_4kg, total_kg, monthly_gas_price, amount, raw, synced_at)
    values (
      item->>'id',
      item->>'companyId',
