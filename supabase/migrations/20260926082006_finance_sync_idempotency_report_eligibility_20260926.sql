...
BEGIN
  ...
  FOREACH v_collection IN ARRAY ARRAY['incomes','expenses','bankTransactions','journalEntries','journalLines'] LOOP
    SELECT item_id INTO v_duplicate_id
    FROM (
      SELECT item->>'id' AS item_id, count(*) AS item_count
      FROM jsonb_array_elements(p_payload->v_collection) item
      GROUP BY item->>'id'
    ) grouped_ids
    WHERE coalesce(item_id, '') = '' OR item_count > 1
    LIMIT 1;
    IF FOUND THEN
      RAISE EXCEPTION 'finance sync payload contains missing or duplicate ids in %: %', v_collection, v_duplicate_id
        USING ERRCODE = '23505';
    END IF;
  END LOOP;
  ...
  WHERE item->>'syncSource' = 'shenglong'
     OR EXISTS (
       SELECT 1 FROM jsonb_array_elements(p_payload->'incomes') incoming
  ...
  FOREACH v_collection IN ARRAY ARRAY['incomes','expenses','bankTransactions','journalEntries','journalLines'] LOOP
    ...
    FROM (
      SELECT item->>'id' AS item_id, count(*) AS item_count
      FROM jsonb_array_elements(coalesce(v_next->v_collection, '[]'::jsonb)) item
      GROUP BY item->>'id'
    ) grouped_ids
    WHERE coalesce(item_id, '') = '' OR item_count > 1
    LIMIT 1;
    IF FOUND THEN
      RAISE EXCEPTION 'finance sync would create missing or duplicate ids in %: %', v_collection, v_duplicate_id
        USING ERRCODE = '23505';
    END IF;
  END LOOP;

  FOREACH v_collection IN ARRAY ARRAY['incomes','expenses'] LOOP
    SELECT coalesce(jsonb_agg(jsonb_set(
      normalized.item,
      '{effectiveForReport}',
      to_jsonb(
        coalesce(normalized.item->>'status', 'approved') = 'approved'
        AND coalesce(normalized.item->>'correctionStatus', '') NOT IN ('corrected','superseded')
        AND coalesce(normalized.item->>'correctionType', '') NOT IN ('reversal','void')
      ),
      true
    )), '[]'::jsonb)
    INTO v_existing_incomes
    FROM (
      SELECT CASE
        WHEN item->>'correctionStatus' = 'corrected' AND nullif(item->>'correctionType', '') IS NULL
        ...
        WHEN nullif(item->>'correctionType', '') IS NOT NULL AND nullif(item->>'correctionStatus', '') IS NULL
    ...
    v_next := jsonb_set(v_next, ARRAY[v_collection], v_existing_incomes, true);
  END LOOP;
